    ...
    switch (msg.type) {
      case "scoreUpdated": {
        ...
        setMatches((prevMatches) =>
            prevMatches.map((m) =>
                String(m.id) === String(updatedMatch.matchId)
                    ? {
                      ...m,
      ...
      case "matchUpdated": {
        const { commentary: newCommentary, match } = msg.data;

          // Update matches list (score)
          setMatches((prevMatches) =>
              prevMatches.map((m) =>
                  String(m.id) === String(match.id) ? match : m
              )
          );

        // Update commentary only if this match is active
        if (newCommentary && latestMatchIdRef.current != null &&
            String(match.id) === String(latestMatchIdRef.current)) {

          const normalized = {
            ...newCommentary,
            createdAt:
                newCommentary.createdAt ?? new Date().toISOString(),
          };
